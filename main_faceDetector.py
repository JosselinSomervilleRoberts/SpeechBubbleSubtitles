...
refresh_recognition = 50 # Evevy x frames, we try face_recognition again on this character
debug_time = False
debug = False


def recognize2(x):
    print("recognize2 avec", x, frame_index)
    return x
...
if __name__ == '__main__':
    ...
    with mp_face_mesh.FaceMesh(
      ...
      while cap.isOpened():
        ...
        if debug_time: print("Before mesh:", 1000*(time.time()-t0), "ms")
        results = face_mesh.process(image)
        if debug_time: print("After frame:", 1000*(time.time()-t0), "ms")
        ...
        if results.multi_face_landmarks:
            ...
            if debug_time: print("Before arrays:", 1000*(time.time()-t0), "ms")
            ...
            if debug_time: print("After arrays:", 1000*(time.time()-t0), "ms")
            ...
            while continuer and (len(available_faces_indexes) > 0) and (len(boxes) > 0):
                ...
                if debug_time: print("sima_mt", sim_mat)
                ...
                if len(sim_mat.shape) > 1:
                    ...
                    if sim_mat[index_max] > similarity_threshold:
                        if debug: print("- Detected", index_max[1], "assigned to", index_max[0])
                        
                        if ((frame_index + index_max[0]) % refresh_recognition == 0) and faces[existing_index].name.lower() == "unknown":
                            
    
                            if debug_time: print("Before recognition:", 1000*(time.time()-t0), "ms")
                            pool.apply_async(recognize, (image, boxes[detected_index], faces[existing_index], frame_index, ), callback=mycallback)
                            #pool.apply_async(recognize2, (frame_index,), callback=mycallback)
                            #recognize(image, boxes[detected_index], faces[existing_index])
                            if debug_time: print("After recognition:", 1000*(time.time()-t0), "ms")
            ...
            for index, box in enumerate(boxes):
                if debug: print("WARNING NEW FACE")
                p = Perso()
                p.detected(box, landmarks[index], frame_index)    
                
                try:
                    pool.apply_async(recognize, (image, box, p, frame_index, ), callback=mycallback)
        ...
        if debug_time: print("Compute frame:", 1000*(time.time()-t0), "ms")
        cv2.imshow('Main FacesDetector', image)
        if debug_time: print("Show frame:", 1000*(time.time()-t0), "ms")
